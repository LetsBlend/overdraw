...
fn main() {
    ...
    let texture = Texture2D::new(window.get_width(), window.get_height());
    ...
    while window.open() {
        ...
        if !is_hovered {
            paint(&input, &compute_shader, &texture, color_picked, cursor_coords, prev_cursor_coords, brush_size);
            if input.get_key(Keycode::LAlt) {
                /////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                // This is only here because there is a bug in the input library, and I am too lazy to properly fix it
        ...
        // Gui
        is_hovered = gui.show_color_picker(&mut window.p_window, &mut color_picked, &mut first_click);
...
fn paint(input: &Input, compute_shader: &ComputeShader, texture: &Texture2D, color_picked: Vec4, cursor_coords: Vec2, prev_cursor_coords: Vec2, brush_size: i32) {
    if !input.get_key(Keycode::LAlt) && (input.get_button(1) || input.get_button(2)) {
